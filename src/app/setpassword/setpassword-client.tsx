...
export default function SetPasswordClient({ tenant, token, nextPath = null }: Props) {
  ...
  useEffect(() => {
    if (!tenant || !token) {
      setErr("Invalid or incomplete activation link. Please request a new email.");
    }
  }, [tenant, token]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Success → go to nextPath if internal, otherwise to login with tenant prefilled
      const dest =
        nextPath && nextPath.startsWith("/")
          ? nextPath
          : `/login?activated=1&tenant=${encodeURIComponent(tenant)}`;
      router.replace(dest);
    } catch (e: any) {
